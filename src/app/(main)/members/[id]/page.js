                ...
                <section id="message" className="rounded-2xl p-4 space-y-3" style={{ background: 'var(--color-bg-card)', border: 'var(--card-border)' }}>
                    ...
                    <form onSubmit={sendMessage} className="space-y-2">
                        <div className="flex gap-2">
                            ...
                            <button type="button" onClick={() => fileInputRef.current?.click()} className="h-11 w-11 rounded-2xl bg-primary/10 text-primary flex items-center justify-center" aria-label="Attach image"><ImagePlus size={18} /></button>
                            <button type="button" onClick={toggleRecording} className={`h-11 w-11 rounded-2xl flex items-center justify-center ${recording ? 'bg-danger text-white' : 'bg-sky-100 text-sky-700'}`} aria-label="Record voice note">{recording ? <StopCircle size={18} /> : <Mic size={18} />}</button>
                            <button className="h-12 w-12 rounded-2xl gradient-primary text-white flex items-center justify-center" aria-label="Send message"><Send size={18} /></button>
                        </div>
...
function Stat({ icon: Icon, label, value }) {
    return <div className="rounded-2xl p-3 text-center" style={{ background: 'var(--color-bg-card)', border: 'var(--card-border)' }}><Icon size={16} className="mx-auto text-primary mb-1" /><p className="text-[11px] text-text-muted">{label}</p><p className="text-sm font-black text-text-primary">{value}</p></div>;
}
